from pathlib import Path
import json, os, sys, hashlib, datetime, subprocess
R=Path(r"D:\\BannerlordAIResearch")
root=R/r"workspace\\_PatchStaging\\AutonomousOperator_v002158_TransportRegistration_20260921_1650"
runner=root/r"candidate\\BannerlordAITestRunner.dll"
clan=R/r"workspace\\_PatchStaging\\PatrolDefenseRecentOutcomeAge_v02138_20260921_1234\\candidate\\ClanAI.dll"
validator=R/r"workspace\\validate_v02158_provider_transport_registration.py"
source=root/r"candidate\\PatrolDefenseProviderTransportRegistration.cs"

runner_sha=hashlib.sha256(runner.read_bytes()).hexdigest().upper()
clan_sha=hashlib.sha256(clan.read_bytes()).hexdigest().upper()
validator_sha=hashlib.sha256(validator.read_bytes()).hexdigest().upper()
source_sha=hashlib.sha256(source.read_bytes()).hexdigest().upper()

regp=R/r"Automation\\Autopilot\\registry.json"
reg=json.loads(regp.read_text(encoding="utf-8-sig"))
reg.setdefault("actions",{})["v02158_provider_transport_registration"]={
 "enabled":True,
 "description":"v0.2.10.58 runtime provider transport-request registration / transportRequestId shadow.",
 "command":["python","-X","utf8",str(validator)],
 "required_candidate_version":"v0.2.10.58-provider-transport-registration-shadow",
 "required_next_action_contains":"v0.2.10.58",
 "max_attempts":1,
 "next_action_id":None,
 "owner":"coder",
 "work_class":"LIVE_GAME",
 "may_launch_game":True,
 "requires_analyzer_clear":True
}
tmp=regp.with_suffix(".json.tmp")
tmp.write_text(json.dumps(reg,indent=2)+"\n",encoding="utf-8")
os.replace(tmp,regp)

curp=R/r"Longitudinal\\EvidenceIndex\\HandoffV3\\current.json"
cur=json.loads(curp.read_text(encoding="utf-8-sig"))
patch={
 "active_candidate":{
  "status":"EXECUTE_READY",
  "candidate_clanai":str(clan),
  "candidate_runner":str(runner),
  "clanai_test_sha256":clan_sha,
  "runner_test_sha256":runner_sha,
  "validator":str(validator),
  "validator_sha256":validator_sha,
  "transport_registration_source_sha256":source_sha,
  "compile_result":"PASS_RUNNER_0_ERRORS_CLANAI_REUSED_V02138",
  "isolated_fixture_result":"PASS_FIXTURES checks=225",
  "provider_request_contract_fixture_result":"PASS_FIXTURES checks=49",
  "transport_registration_policy":"ONE_EXACT_TRANSPORT_REQUEST_PER_REGISTERED_PROVIDER_REQUEST_IDEMPOTENT_REPLAY",
  "network_model_policy":"NO_NETWORK_NO_MODEL_CALL"
 },
 "next_action":"Coder EXECUTE v0.2.10.58 bounded live provider-transport registration gate. Natural request -> claim attempt-1 -> exact provider-request registration -> exact deterministic_local_loopback transport request registration with runtime SHA-256 transportRequestId -> exact replay idempotent; provider invocation/result streams remain empty, no network/model/Apply authority, full cleanup.",
 "blockers":[]
}
pp=R/r"workspace\\v02158_transport_registration_execute_route.json"
pp.write_text(json.dumps(patch,indent=2)+"\n",encoding="utf-8")
cp=subprocess.run([
 sys.executable,"-X","utf8",str(R/r"Tools\\Handoff\\handoff_checkpoint.py"),
 "checkpoint","--patch-file",str(pp),
 "--event-type","PLAN_COMPLETE",
 "--message","v0.2.10.58 runner compiles 0 errors; transport registration fixture suite 225/225 PASS; provider-request contract fixtures 49/49 PASS; validator pycompile PASS; route exact transportRequestId registration/idempotent replay live gate.",
 "--action-id","v02158-transport-registration-live-route",
 "--expected-seq",str(cur["checkpoint_seq"])
],capture_output=True,text=True)
print(cp.stdout)
if cp.returncode:
    print(cp.stderr)
    raise SystemExit(cp.returncode)

subprocess.run([sys.executable,"-X","utf8",str(R/r"Tools\\Autopilot\\sync_thinking_loop.py")],check=True)
sp=R/r"Automation\\Autopilot\\state.json"
s=json.loads(sp.read_text(encoding="utf-8-sig"))
s.update({
 "enabled":True,
 "status":"READY",
 "attempts":0,
 "runner_pid":None,
 "pending_action_id":"v02158_provider_transport_registration",
 "last_result":None,
 "return_code":None,
 "last_supervisor_result":None,
 "rearmed_reason":"v0.2.10.58 build+225+49 fixtures+validator pass; bounded transport registration live gate ready.",
 "rearmed_at":datetime.datetime.now().astimezone().isoformat()
})
tmp=sp.with_suffix(".json.tmp")
tmp.write_text(json.dumps(s,indent=2)+"\n",encoding="utf-8")
os.replace(tmp,sp)
