...
import provider_execution_package_v02167_clean as execution_package
...
OWNER = "sol_execution_package_v02167"
SOURCE_PREFIX = "ClanAI V02167 EXECUTION PACKAGE SOURCE "
...
PACKAGE_FIXTURE = ROOT / r"workspace\test_provider_execution_package_v02167.py"
...
def main():
    ...
    static_checks = {
        ...
        "package_schema": "BannerlordAI.ProviderExecutionPackage.v1" in package_source,
        "package_builder": "def build_execution_package(" in package_source,
        "package_validator": "def validate_execution_package(" in package_source,
        "package_adapter": "class DeterministicProviderAdapter" in package_source,
        "package_no_secret_read": "os.environ" not in package_source and "getenv(" not in package_source,
        "package_no_network": "requests." not in package_source and "httpx" not in package_source and "socket." not in package_source,
        "package_no_model_api": "OpenAI" not in package_source and "ChatCompletion" not in package_source,
    }
    ...
    package_fixture = run_text(
        [sys.executable, "-X", "utf8", str(PACKAGE_FIXTURE)],
        "PASS_FIXTURES checks=44",
    )
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02167_ExecutionPackage_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_ExecutionPackage_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "execution-package-v02167-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.ExecutionPackageValidation.v1",
    ...
    try:
        h.base.guard(
            "acquire",
            OWNER,
            "patrol_defense_execution_package_shadow",
            h.OLD_CLAN,
            operation,
            "1800",
        )
        ...
        if row is None:
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02167 execution-package environmental inconclusive",
            )
            run = None
        else:
            ...
            stale_package = execution_package.build_execution_package(
                request_json,
                transport_json,
                policy_json,
                authorization1,
            ...
            package_stale_checks = {
                ...
                "latest_ordinal_reason":
                    ...
                    in stale_package_validation.reasons,
                "adapter_not_invoked": stale_adapter.adapter_invoked is False,
                "adapter_count_zero": adapter.invocations == 0,
                "no_outcome": stale_adapter.transport_outcome is None,
            }

            exact_package = execution_package.build_execution_package(
                request_json,
                transport_json,
                policy_json,
                authorization2,
            ...
            if (
                not adapter_result.adapter_invoked
                or adapter_result.transport_outcome is None
                or not adapter_result.transport_outcome.success
            ):
                raise RuntimeError(
            ...
            outcome = adapter_result.transport_outcome
            ...
            package_checks = {
                ...
                "adapter_invoked": adapter_result.adapter_invoked is True,
                "adapter_count_one": adapter.invocations == 1,
            }
            ...
            evidence["stale_execution_package_validation"]={
                ...
                "adapterInvoked": stale_adapter.adapter_invoked,
            }
            ...
            evidence["execution_package_validation"]={
                ...
                "adapterInvoked": adapter_result.adapter_invoked,
                "adapterInvocationCount": adapter.invocations,
            }
        ...
        print(json.dumps({
            ...
            "adapterInvocationCount": None if row is None else adapter.invocations,
