...
RUNNER_SRC = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002164_MaxAttemptsPolicy_20260921_2218\candidate"
CLAN_SHA = v46.CLAN_SHA
RUNNER_SHA = "12C265F313341C7BC37067110403CB7F650E16EC41A446FF646496C10A2D08F4"
OWNER = "sol_max_attempts_policy_v02164"
SOURCE_PREFIX = "ClanAI V02164 MAX ATTEMPTS POLICY SOURCE "

MAIN_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002164_MaxAttemptsPolicy_20260921_2218\fixtures\Fixtures.csproj"
V3_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002164_MaxAttemptsPolicy_20260921_2218\fixtures_v3\FixturesV3.csproj"
V4_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002164_MaxAttemptsPolicy_20260921_2218\fixtures_v4\FixturesV4.csproj"
LOOPBACK_FIXTURE = ROOT / r"workspace\test_provider_transport_loopback_v02164.py"
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02164_MaxAttemptsPolicy_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_MaxAttemptsPolicy_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "max-attempts-policy-v02164-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.MaxAttemptsPolicyValidation.v1",
    ...
    try:
        h.base.guard(
            "acquire",
            OWNER,
            "patrol_defense_max_attempts_policy_shadow",
            h.OLD_CLAN,
            operation,
            "1800",
        )
        ...
        run = h.base.bus.start_run(
            issuer="max_attempts_policy_live",
        ...
        if row is None:
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02164 max-attempts policy environmental inconclusive",
            )
            run = None
        else:
            ...
            if not all(checks.values()):
                raise RuntimeError(
                    "v02164 live checks " + json.dumps(checks, sort_keys=True)
                )
            ...
            evidence["result"] = "PASS_MAX_ATTEMPTS_POLICY_ENFORCEMENT_SHADOW"
            h.base.atomic_json(validation / "validation.json", evidence)
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02164 max-attempts policy verified",
            )
            run = None
        ...
        return 0 if evidence["result"] == "PASS_MAX_ATTEMPTS_POLICY_ENFORCEMENT_SHADOW" else 2
        ...
        if run is not None and h.base.process_pids():
            try:
                h.safe_exit(
                    ...
                    token,
                    run,
                    "v02164 max-attempts policy failure",
                )
                run = None
        ...
        if run is not None and not h.base.process_pids():
            try:
                h.base.bus.close_run("v02164 pre-ready failure")
                run = None
            except Exception as e:
