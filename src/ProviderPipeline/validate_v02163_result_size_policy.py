...
RUNNER_SRC = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002163_ResultSizePolicy_20260921_2208\candidate"
...
MAIN_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002163_ResultSizePolicy_20260921_2208\fixtures\Fixtures.csproj"
V3_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002163_ResultSizePolicy_20260921_2208\fixtures_v3\FixturesV3.csproj"
V4_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002163_ResultSizePolicy_20260921_2208\fixtures_v4\FixturesV4.csproj"
LOOPBACK_FIXTURE = ROOT / r"workspace\test_provider_transport_loopback_v02163.py"
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02163_ResultSizePolicy_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_ResultSizePolicy_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "result-size-policy-v02163-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.ResultSizePolicyValidation.v1",
    ...
    try:
        ...
        if row is None:
            ...
            evidence["result"] = "PASS_RESULT_SIZE_POLICY_ENFORCEMENT_SHADOW"
            h.base.atomic_json(validation / "validation.json", evidence)
        ...
        return 0 if evidence["result"] == "PASS_RESULT_SIZE_POLICY_ENFORCEMENT_SHADOW" else 2
        ...
        if run is not None and not h.base.process_pids():
            try:
                h.base.bus.close_run("v02163 pre-ready failure")
                run = None
            except Exception as e:
