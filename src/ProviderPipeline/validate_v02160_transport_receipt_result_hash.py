...
RUNNER_SRC = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002160_TransportReceiptResultHash_20260921_1948\candidate"
...
BACKCOMPAT_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002160_TransportReceiptResultHash_20260921_1948\fixtures\Fixtures.csproj"
V3_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002160_TransportReceiptResultHash_20260921_1948\fixtures_v3\FixturesV3.csproj"
...
def main():
    ...
    static_checks = {
        ...
        "binding_command": "PATROL_DEFENSE_PROVIDER_TRANSPORT_RESULT_ADMIT "
        in runner_source,
    ...
    if v3_fixture.returncode != 0 or "PASS_V3_FIXTURES checks=63" not in v3_fixture_result:
        raise RuntimeError(
            "v3 fixtures failed "
            + v3_fixture_result
            + " "
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02160_TransportReceiptResultHash_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_TransportReceiptResultHash_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "transport-receipt-result-hash-v02160-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.TransportReceiptResultHashValidation.v1",
    ...
    try:
        ...
        if row is None:
            ...
            evidence["transport_result_binding"] = binding
        ...
        return 0 if evidence["result"] == "PASS_TRANSPORT_RECEIPT_RESULT_HASH_BINDING_SHADOW" else 2
        ...
        if run is not None and h.base.process_pids():
            try:
                h.safe_exit(OWNER, token, run, "v02160 receipt/result hash failure")
                run = None
        ...
        if run is not None and not h.base.process_pids():
            try:
                h.base.bus.close_run("v02160 pre-ready failure")
                run = None
            except Exception as e:
