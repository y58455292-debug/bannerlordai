...
RUNNER_SRC = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002158_TransportRegistration_20260921_1650\candidate"
CLAN_SHA = v46.CLAN_SHA
RUNNER_SHA = "B7D93EBADF946A3781EF89BF71BA567B74F89C23A30680F4794D0A0FEA98A657"
OWNER = "sol_provider_transport_registration_v02158"
SOURCE_PREFIX = "ClanAI V02158 TRANSPORT REG SOURCE "
FIXTURE_PROJECT = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002158_TransportRegistration_20260921_1650\fixtures\Fixtures.csproj"
CONTRACT_FIXTURE = ROOT / r"workspace\test_provider_request_contract_v02154.py"
OUTBOX = v46.OUTBOX
CLAIMS = v50.CLAIMS
REQUEST_REGISTRATIONS = v55.REGISTRATIONS
TRANSPORT_REGISTRATIONS = ROOT / r"Automation\TestRunner\patrol_defense_provider_transport_registrations.jsonl"
...
def transport_envelope(registration, request_envelope):
    envelope = {
        ...
        "requestInputSha256": request_envelope["inputSha256"],
    }
    raw = json.dumps(envelope, separators=(",", ":"), ensure_ascii=False)
    return envelope, raw


def transport_checks(row, envelope, expected_id, idempotent):
    expected_reason = (
    ...
    return {
        ...
        "transport_request_id": row.get("transportRequestId") == expected_id,
        "transport": row.get("transportId") == envelope.get("transportId"),
        ...
        == envelope.get("requestInputSha256"),
...
def main():
    ...
    transport_source = (
        RUNNER_SRC / "PatrolDefenseProviderTransportRegistration.cs"
    ).read_text(encoding="utf-8", errors="replace")

    static_checks = {
        "transport_class": "PatrolDefenseProviderTransportRegistration"
        in transport_source,
        "transport_request_schema":
        "BannerlordAI.PatrolDefenseProviderTransportRequest.v1" in transport_source,
        "transport_receipt_schema":
        "BannerlordAI.PatrolDefenseProviderTransportRegistration.v1" in transport_source,
        "register_method": "RegisterTransportRequest(" in dispatch_source,
        "transport_job_id": "TransportRequestId" in dispatch_source,
        "transport_job_transport": "TransportId" in dispatch_source,
        "command": "PATROL_DEFENSE_PROVIDER_TRANSPORT_REGISTER " in runner_source,
        "path": "patrol_defense_provider_transport_registrations.jsonl"
        in runner_source,
        "release_clears": "job.TransportRequestId = null;" in dispatch_source
        and "job.TransportId = null;" in dispatch_source,
        "provider_request_required": "PROVIDER_REQUEST_NOT_REGISTERED"
        in dispatch_source,
        "provider_request_id_match": "PROVIDER_REQUEST_ID_MISMATCH"
        in dispatch_source,
        "provider_model_match": "PROVIDER_REQUEST_MODEL_MISMATCH"
        in dispatch_source,
        "provider_input_sha_match": "PROVIDER_REQUEST_INPUT_SHA_MISMATCH"
        in dispatch_source,
        "sha256": "SHA256.Create()" in transport_source,
        "no_http": "HttpClient" not in transport_source,
        "no_socket": "Socket" not in transport_source,
        "no_openai": "OpenAI" not in transport_source,
        "no_model_api": "ChatCompletion" not in transport_source,
        "no_apply": "PatrolDefenseApplyGate" not in transport_source,
        "no_score": "SetBehaviorScore" not in transport_source,
        "no_memory_write": "RecordPatrolDefenseEpisode" not in transport_source,
    }
    ...
    if fixture.returncode != 0 or "PASS_FIXTURES checks=225" not in fixture_result:
        raise RuntimeError(
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02158_ProviderTransportRegistration_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_ProviderTransportRegistration_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "provider-transport-registration-v02158-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.ProviderTransportRegistrationValidation.v1",
    ...
    try:
        h.base.guard(
            "acquire",
            OWNER,
            "patrol_defense_provider_transport_registration_shadow",
            h.OLD_CLAN,
            operation,
            "1800",
        )
        ...
        run = h.base.bus.start_run(
            issuer="provider_transport_registration_live",
        ...
        if row is None:
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02158 transport registration environmental inconclusive",
            )
            run = None
        else:
            ...
            if not all(request_reg_checks.values()):
                raise RuntimeError(
                    "provider request registration checks "
                    + json.dumps(request_reg_checks, sort_keys=True)
                )

            transport, transport_json = transport_envelope(
                request_registration,
                request_envelope,
            )
            ...
            first_transport = transport_rows[-1]
            first_checks = transport_checks(
                first_transport,
                transport,
            ...
            before_replay = len(transport_rows)
            h.send_wait(
                OWNER,
                run,
                transport_command,
            ...
            replay_rows = h.read_jsonl(TRANSPORT_REGISTRATIONS)
            if len(replay_rows) <= before_replay:
                raise RuntimeError("transport registration replay receipt missing")
            replay = replay_rows[-1]
            replay_checks = transport_checks(
                replay,
                transport,
                expected_transport_request_id,
                True,
            ...
            checks["transport_id_independent"] = (
                first_transport.get("transportRequestId")
                == hashlib.sha256(transport_json.encode("utf-8")).hexdigest().upper()
            )
            checks["provider_invocation_rows_empty"] = (
                len(h.read_jsonl(PROVIDER_INVOCATIONS)) == 0
            )
            checks["provider_result_v1_rows_empty"] = len(h.read_jsonl(RESULT_V1)) == 0
            checks["provider_result_v2_rows_empty"] = len(h.read_jsonl(RESULT_V2)) == 0
            checks["no_apply_side_effects"] = h.no_apply_side_effects()

            if not all(checks.values()):
                raise RuntimeError(
                    "provider transport registration live checks "
            ...
            evidence["transport_request_envelope_json"] = transport_json
            evidence["transport_registration"] = first_transport
            evidence["transport_registration_replay"] = replay
            evidence["independent_transport_request_id"] = expected_transport_request_id
            evidence["live_checks"] = checks
            evidence["result"] = "PASS_PROVIDER_TRANSPORT_REGISTRATION_SHADOW"
            h.base.atomic_json(validation / "validation.json", evidence)
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02158 provider transport registration verified",
            )
            run = None
            ...
            json.dumps(
                {
                    ...
                    "transportRequestId": None
                    if row is None
                    else first_transport.get("transportRequestId"),
                    "transportId": None
                    if row is None
                    else first_transport.get("transportId"),
                    "replayReason": None if row is None else replay.get("reason"),
                },
                indent=2,
            )
        )
        return 0 if evidence["result"] == "PASS_PROVIDER_TRANSPORT_REGISTRATION_SHADOW" else 2
        ...
        if run is not None and h.base.process_pids():
            try:
                h.safe_exit(OWNER, token, run, "v02158 transport registration failure")
                run = None
        ...
        if run is not None and not h.base.process_pids():
            try:
                h.base.bus.close_run("v02158 pre-ready failure")
                run = None
            except Exception as e:
