...
RUNNER_SRC = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002165_TimeoutBudget_20260921_2317\candidate"
CLAN_SHA = v46.CLAN_SHA
RUNNER_SHA = "5E92F65137C6DBDF114BD77613CE5F384E7B240B496ACC6D7C7E39BDA07ADEB5"
OWNER = "sol_timeout_budget_v02165"
SOURCE_PREFIX = "ClanAI V02165 TIMEOUT BUDGET SOURCE "

MAIN_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002165_TimeoutBudget_20260921_2317\fixtures\Fixtures.csproj"
V3_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002165_TimeoutBudget_20260921_2317\fixtures_v3\FixturesV3.csproj"
V4_FIXTURES = ROOT / r"workspace\_PatchStaging\AutonomousOperator_v002165_TimeoutBudget_20260921_2317\fixtures_v4\FixturesV4.csproj"
LOOPBACK_FIXTURE = ROOT / r"workspace\test_provider_transport_loopback_v02165.py"
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / (
        "PatrolDefense_v02165_TimeoutBudget_" + stamp
    )
    validation.mkdir(parents=True)
    backup = ROOT / "workspace/_PatchBackups" / (
        "Before_TimeoutBudget_" + stamp
    )
    backup.mkdir(parents=True)

    operation = "timeout-budget-v02165-" + stamp
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest().upper()

    evidence = {
        "schema": "BannerlordAI.TimeoutBudgetBindingValidation.v1",
    ...
    try:
        h.base.guard(
            "acquire",
            OWNER,
            "patrol_defense_timeout_budget_binding_shadow",
            h.OLD_CLAN,
            operation,
            "1800",
        )
        ...
        run = h.base.bus.start_run(
            issuer="timeout_budget_binding_live",
        ...
        if row is None:
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02165 timeout-budget binding environmental inconclusive",
            )
            run = None
        else:
            ...
            if len(authorization_rows) <= before_authorization:
                raise RuntimeError("transport execution authorization missing")
            authorization = authorization_rows[-1]
            authorization_checks = {
                "schema": authorization.get("schema")
                == "BannerlordAI.PatrolDefenseProviderTransportExecutionAuthorization.v1",
                "fingerprint": authorization.get("requestFingerprint") == fingerprint,
                "execution_policy": authorization.get("executionPolicyId")
                == expected_policy_id,
                "authorized": authorization.get("authorized") is True,
                "reason": authorization.get("reason")
                == "EXECUTION_ATTEMPT_AUTHORIZED",
                "ordinal": authorization.get("attemptOrdinal") == 1,
                "count": authorization.get("attemptCount") == 1,
                "max_attempts": authorization.get("maxAttempts") == 1,
                "timeout_ms": authorization.get("timeoutMs") == int(v61.TEST_TIMEOUT_MS),
                "authorization_id": isinstance(
                    authorization.get("transportExecutionAuthorizationId"), str
                )
                and len(authorization.get("transportExecutionAuthorizationId")) == 64,
                "network_false": authorization.get("externalNetworkUsed") is False,
                "model_false": authorization.get("modelInvoked") is False,
                "execution_false": authorization.get("executionAuthorized") is False,
                "behavior_false": authorization.get("behaviorMutation") is False,
                "intent_false": authorization.get("intentMutation") is False,
                "score_false": authorization.get("scoreMutation") is False,
                "movement_zero": authorization.get("nativeMovementCalls") == 0,
            }
            ...
            checks.update({
                "authorization_" + k: v
                for k, v in authorization_checks.items()
            })
            ...
            checks["authorization_rows_exactly_one"] = (
                len(h.read_jsonl(AUTHORIZATIONS)) == before_authorization + 1
            )
            ...
            if not all(checks.values()):
                raise RuntimeError(
                    "v02165 live checks " + json.dumps(checks, sort_keys=True)
                )
            ...
            evidence["transport_execution_authorization"] = authorization
            evidence["transport_execution_authorization_rows"] = len(
                h.read_jsonl(AUTHORIZATIONS)
            )
            evidence["timeout_budget"] = {
                "registeredTimeoutMs": policy_registration.get("timeoutMs"),
                "authorizedTimeoutMs": authorization.get("timeoutMs"),
                "bound": authorization.get("timeoutMs") == int(v61.TEST_TIMEOUT_MS),
            }
            ...
            evidence["result"] = "PASS_TIMEOUT_BUDGET_BINDING_SHADOW"
            h.base.atomic_json(validation / "validation.json", evidence)
            ...
            h.safe_exit(
                ...
                token,
                run,
                "v02165 timeout-budget binding verified",
            )
            run = None
        ...
        print(json.dumps({
            ...
            "timeoutMs": None if row is None else authorization.get("timeoutMs"),
        ...
        return 0 if evidence["result"] == "PASS_TIMEOUT_BUDGET_BINDING_SHADOW" else 2
        ...
        if run is not None and h.base.process_pids():
            try:
                h.safe_exit(
                    ...
                    token,
                    run,
                    "v02165 timeout-budget binding failure",
                )
                run = None
        ...
        if run is not None and not h.base.process_pids():
            try:
                h.base.bus.close_run("v02165 pre-ready failure")
                run = None
            except Exception as e:
