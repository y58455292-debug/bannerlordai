...
root=R/r"workspace\_PatchStaging\AutonomousOperator_v002145_ProviderBoundary_20260921_1416"
runner=root/r"candidate\BannerlordAITestRunner.dll"
validator=R/r"workspace\validate_v02145_provider_boundary.py"
...
reg.setdefault("actions",{})["v02145_provider_boundary"]={
 "enabled":True,
 "description":"v0.2.10.45 deterministic local cognition-provider interface behind exact admission firewall.",
 "command":["python","-X","utf8",str(validator)],
 "required_candidate_version":"v0.2.10.45-deliberation-provider-boundary-shadow",
 "required_next_action_contains":"v0.2.10.45",
 "max_attempts":1,
 "next_action_id":None,
 "owner":"coder",
...
patch={
 "active_candidate":{
  ...
  "provider_mode":"DETERMINISTIC_LOCAL_MOCK_NO_NETWORK"
 },
 "next_action":"Coder EXECUTE v0.2.10.45 bounded live provider-boundary gate. Require one natural route-eligible Manan request, invoke deterministic local mock KEEP_BASELINE for exact fingerprint, verify providerInvoked=true and strict admission admitted=true with externalNetworkUsed/modelInvoked/executionAuthorized=false, then immediate identical provider replay must not invoke provider and must reject NO_PENDING_ROUTE_REQUEST; no Apply/score/memory authority; EXIT_NOSAVE and full cleanup.",
 "blockers":[]
}
pp=R/r"workspace\v02145_provider_boundary_execute_route.json"
...
cp=subprocess.run([
 ...
 "--message","v0.2.10.45 runner compiles 0 errors; provider interface/mock/runtime fixture suite 97/97 PASS; no-network validator py_compile PASS; route bounded natural request -> mock provider -> strict admission -> replay rejection live gate.",
 "--action-id","v02145-provider-boundary-live-route",
...
s.update({
 ...
 "attempts":0,
 "runner_pid":None,
 "pending_action_id":"v02145_provider_boundary",
 "last_result":None,
 "return_code":None,
 "last_supervisor_result":None,
 "rearmed_reason":"v0.2.10.45 build+97 fixtures+no-network validator pass; bounded provider boundary live gate ready.",
