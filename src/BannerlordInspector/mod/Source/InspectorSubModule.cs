...
namespace BannerlordInspector
{
    ...
    public class InspectorSubModule : MBSubModuleBase
    {
        ...
        private long _applicationFrame;
            ...
            IGameStarter gameStarterObject)
        {
            ...
            {
                starter.AddBehavior(
                    new RearSecurityBattleObserver());
            }
        }
        ...
        protected override void OnApplicationTick(float dt)
        {
            ...
            {
                // AI research mode keeps only the two broad world views that
                // materially help current campaign-AI experiments. They are
                // staggered so diagnostics do not compete with Bannerlord's
                // campaign simulation every rendered frame.
                if ((_applicationFrame % 8) == 0)
                    WorldCensus.Tick();

                if ((_applicationFrame % 60) == 0)
                    LordPartyCensus.Tick();
            }
            else
            {
                // Full diagnostic mode retains the broad inspector surface
                // for economy/content investigations when explicitly needed.
                WorldCensus.Tick();
                EconomicCensus.Tick();
                SupplyCensus.Tick();
                TrafficCensus.Tick();
    ...
    public static class InspectorConfig
    {
        ...
        /// <summary>
        /// Performance-first mode for long campaign-AI tests. Heavy unrelated
        /// censuses are paused; event-driven AI traces and rare-event observers
        /// remain active.
        /// </summary>
        public static bool AiResearchLightMode = true;
            ...
            try
            {
                if (!File.Exists(path))
                {
                    ...
                    File.WriteAllText(path, string.Join("\n", new[]
                    {
                        ...
                        "collectErrors=true",
                        "",
                        "# Performance-first mode for long campaign-AI tests.",
                        "# Keeps AI/event probes active but pauses unrelated economy/market/history scans.",
                        "aiResearchLightMode=true",
                        ""
                    }));
                ...
                foreach (string raw in File.ReadAllLines(path))
                {
                    ...
                    if (key == "aiResearchLightMode" && bool.TryParse(value, out bool light)) AiResearchLightMode = light;
                }
            }
            catch (Exception ex)
            {
