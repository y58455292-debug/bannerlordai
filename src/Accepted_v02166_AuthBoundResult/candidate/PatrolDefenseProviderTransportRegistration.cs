...
{
    internal sealed class PatrolDefenseProviderTransportRegistrationData
    {
    ...
    internal static class PatrolDefenseProviderTransportRegistration
    {
        ...
        private static bool TryReadHex4(string text, ref int index, out char value)
        {
            value = '\0';
        ...
        private static bool TryReadJsonString(string text, ref int index, out string value)
        {
            ...
            while (index < text.Length)
            {
                ...
                switch (escape)
                {
                    ...
                    case 'u':
                        char unicode;
                        if (!TryReadHex4(text, ref index, out unicode))
                            return false;
                        sb.Append(unicode);
                        break;
                    default:
        ...
        internal static PatrolDefenseProviderTransportRegistrationData Evaluate(
            PatrolDefenseProviderDispatchQueue queue,
            string envelopeJson)
        {
            PatrolDefenseProviderTransportRegistrationData data =
                new PatrolDefenseProviderTransportRegistrationData();

            Dictionary<string, string> fields;
            if (!TryParseFlatStringObject(envelopeJson, out fields))
            {
            ...
            foreach (KeyValuePair<string, string> pair in fields)
            {
                ...
                {
                    data.Reason = "UNEXPECTED_FIELD:" + pair.Key;
                    return data;
                }
            }
            ...
            foreach (string field in AllowedFields)
            {
                ...
                {
                    data.Reason = "MISSING_FIELD:" + field;
                    return data;
                }
            }

            if (!string.Equals(fields["schema"], RequestSchema, StringComparison.Ordinal))
            {
                data.Reason = "SCHEMA_INVALID";
                return data;
            }

            data.TransportId = fields["transportId"];
            data.ProviderRequestId = fields["providerRequestId"];
            data.ProviderId = fields["providerId"];
            data.ModelId = fields["modelId"];
            data.AttemptId = fields["attemptId"];
            data.RequestFingerprint = fields["requestFingerprint"];
            data.RequestInputSha256 = fields["requestInputSha256"];

            if (!Present(data.TransportId))
            {
                data.Reason = "TRANSPORT_ID_MISSING";
                return data;
            }
            if (!Present(data.ProviderRequestId))
            {
                data.Reason = "PROVIDER_REQUEST_ID_MISSING";
                return data;
            }
            ...
            {
                data.Reason = "REQUEST_FINGERPRINT_MISSING";
                return data;
            }
            if (!Present(data.RequestInputSha256))
            {
                data.Reason = "REQUEST_INPUT_SHA256_MISSING";
                return data;
            }

            data.TransportRequestId =
                Sha256Upper(Encoding.UTF8.GetBytes(envelopeJson));

            if (queue == null)
            {
                data.Reason = "DISPATCH_QUEUE_MISSING";
                return data;
            }

            PatrolDefenseProviderTransportRegistrationResult registration =
                queue.RegisterTransportRequest(
            ...
            data.Idempotent = registration.Idempotent;
            data.Reason = registration.Reason;
            data.QueueCount = registration.QueueCount;
            return data;
        }
            ...
            PatrolDefenseProviderTransportRegistrationData data)
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append("{");
            sb.Append("\"schema\":\"BannerlordAI.PatrolDefenseProviderTransportRegistration.v1\"");
            ...
            sb.Append(data.QueueCount.ToString(CultureInfo.InvariantCulture));
