...
{
    internal sealed class PatrolDefenseProviderTransportReceiptResultV5BindingData
    {
        ...
        internal List<string> RejectionReasons =
            new List<string>();
        internal PatrolDefenseProviderResultV5AdmissionData
            ProviderResultAdmission;
    }

    internal static class PatrolDefenseProviderTransportReceiptResultV5Binding
    {
        ...
        private static void Reject(
            PatrolDefenseProviderTransportReceiptResultV5BindingData data,
            string reason)
        {
            if (data != null &&
        ...
        internal static PatrolDefenseProviderTransportReceiptResultV5BindingData
            Evaluate(
                ...
                byte[] resultBytes)
        {
            ...
            data.TransportExecutionAuthorizationId =
                GetString(receipt, "transportExecutionAuthorizationId");
            string receiptExecutionOrdinal =
                GetString(receipt, "transportExecutionAttemptOrdinal");
            if (!int.TryParse(
                    receiptExecutionOrdinal,
                    NumberStyles.None,
                    CultureInfo.InvariantCulture,
                    out data.TransportExecutionAttemptOrdinal) ||
                data.TransportExecutionAttemptOrdinal <= 0)
            {
                Reject(data, "TRANSPORT_RECEIPT_EXECUTION_AUTHORIZATION_ORDINAL_INVALID");
                return data;
            }
            data.ResultSha256 =
            ...
            if (!Present(data.TransportId) ||
                ...
                !Present(data.TransportExecutionAuthorizationId) ||
            ...
            string resultExecutionAuthorizationId =
                GetString(result, "transportExecutionAuthorizationId");
            string resultExecutionOrdinalRaw =
                GetString(result, "transportExecutionAttemptOrdinal");
            int resultExecutionOrdinal;
            if (!int.TryParse(
                    resultExecutionOrdinalRaw,
                    NumberStyles.None,
                    CultureInfo.InvariantCulture,
                    out resultExecutionOrdinal) ||
                resultExecutionOrdinal <= 0)
            {
                Reject(data, "PROVIDER_RESULT_EXECUTION_AUTHORIZATION_ORDINAL_INVALID");
                return data;
            }
            ...
            if (!string.Equals(
                    data.TransportExecutionAuthorizationId,
                    resultExecutionAuthorizationId,
                    StringComparison.Ordinal))
                Reject(data, "RECEIPT_RESULT_EXECUTION_AUTHORIZATION_MISMATCH");

            if (data.TransportExecutionAttemptOrdinal != resultExecutionOrdinal)
                Reject(data, "RECEIPT_RESULT_EXECUTION_AUTHORIZATION_ORDINAL_MISMATCH");
            ...
            PatrolDefenseProviderTransportExecutionAuthorizationMatchResult
                authorizationMatch =
                    queue.MatchRegisteredTransportExecutionAuthorization(
                        data.RequestFingerprint,
                        data.ExecutionPolicyId,
                        data.TransportExecutionAuthorizationId,
            ...
            data.TransportExecutionAuthorizationMatchReason =
                "EXECUTION_AUTHORIZATION_MATCHED";
            ...
            data.ProviderResultAdmission =
                PatrolDefenseProviderResultV5Admission.EvaluateRequestBound(
                    gate,
                    queue,
                    resultJson);
            ...
            PatrolDefenseProviderTransportReceiptResultV5BindingData data)
        {
            ...
            sb.Append("{");
            sb.Append(
                "\"schema\":\"BannerlordAI.PatrolDefenseProviderTransportReceiptResultV5Binding.v1\"");
