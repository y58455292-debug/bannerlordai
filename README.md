...
Phase 5-v1 now also has a bounded **STRONG PASS** runtime characterization. Observation-only telemetry preserved the accepted policy byte-for-byte and was checkpointed before deployment as `08e82ac8cf0f3fc126129fbc77206fb8733ba636`; runtime DLL SHA-256 was `E4DB15DAF5C437F45F559CADDD3F385CE062416D2896EE451826D961D9DDDF7A`. Minimum proof completed naturally after 19.527 campaign hours: secure Syronea at Security 100 changed native weight 1 to 0.75; naturally weak Tepes village resolved Tepes Castle Security 43.85594 and changed native 1 to 1.03072023; Psotai proved the village-bound Security path; and a near-neutral Hertogea sample at Security 50.7775078 produced multiplier 0.996112466. Bannerlord naturally created looter party `looters_50217` through its own bandit party component while Phase 5 only observed the creation event, and a native hideout invocation passed through exactly at weight 0.111111112 with `applied=False`. There were zero telemetry errors and zero save commands; `EXIT_NOSAVE` preserved the protected fixture hash/timestamp, Strategic Commitment remained Observe, Visual War remained OFF, and rollback stayed intact. Phase 5-v1 is **implemented + build-proven + runtime-observed; native looter creation observed: YES; hideout passthrough runtime-observed: YES; balance-proven: NO**. See `Reports/Security/PHASE5_LOCAL_BANDIT_CONTROL_V1_RUNTIME_RESULT.md`.

Phase 6 has now completed its first **offline-only Settlement Society / Vanilla Capability Audit**. The audit covers towns, villages, castles, notable households, gangs/crime, workshops/economy, trade, local issues, recruitment, prisoners, companions/clan members, and local relationships from both player and NPC perspectives. Targeted native inspection found substantial existing NPC infrastructure: opportunistic AI issue resolution, issue effects on settlement/notable models, native governor assignment/effects, active caravan commerce, workshop production/bankruptcy/replacement, alley turnover, shared recruitment, prisoner handling, militia/garrison recovery, and notable affiliation. The strongest bounded gap is decision quality in NPC civic projects: the audited default `BuildingScoreCalculationModel.GetNextDailyBuilding(Town)` chooses randomly even though Festival and Games is an existing native loyalty project. The selected Phase 6-v1 target is therefore **needs-aware Festival and Games selection for idle NPC towns below the selected native rebellious-state loyalty threshold**, while preserving Bannerlord's native 1% consideration cadence, project objects/effects, construction queue, player choices, save state and final commit. This is audit/prioritization only: **Phase 6 gameplay implementation has NOT started; Phase 7 has NOT started.** See `Reports/SettlementSociety/PHASE6_SETTLEMENT_SOCIETY_CAPABILITY_AUDIT.md`.

Phase 6-v1 Civic Project Choice is now implemented and build-proven offline. `CivicProjectSelectionPolicy` is TaleWorlds-free and uses only context validity, NPC-town status, construction-idle status, Festival and Games availability, current loyalty, and the selected native rebellious-state threshold. `CivicProjectBuildingScoreCalculationModel` delegates the audited selected building model, calls native daily selection exactly once, and substitutes only the town's existing native Festival and Games `Building` reference when the strict low-loyalty gate applies. Unknown/foreign building selectors are left untouched; ordinary building selection delegates unchanged; no settlement, queue, project, relation, economy, or save mutation is performed by ClanAI. Twenty-three policy checks, 14 compiled delegation checks, all Phase 6 invariants, all retained Phase 4 and Phase 5 validation, relevant Phase 3 preservation tests, and Release build pass with 0 errors plus the inherited `System.ValueTuple` warning. Offline candidate DLL SHA-256: `2C2C89D5D1086CF119E7D439F08538F4552645A047738B32240144283CD12CE3`. Bannerlord was not launched and the DLL was not deployed. Phase 6-v1 status: **implemented YES; build-proven YES; runtime-observed NO; native final commit observed NO; player-legible NOT YET PROVEN; balance-proven NO**. See `Reports/SettlementSociety/PHASE6_CIVIC_PROJECT_V1_OFFLINE_IMPLEMENTATION_RESULT.md`.

Phase 6-v1 now also has a bounded runtime characterization. The exact selected wrapper was active around the audited `DefaultBuildingScoreCalculationModel`, and Bannerlord's native `BuildingHelper.ChangeDefaultBuilding` commit path was observed after wrapper evaluations. The 155.788-hour run naturally produced two NPC-town controls: Zeonica passed through because construction was active, while idle Marunath at loyalty 94.79668 versus native threshold 25 preserved native `Train Militia` exactly. No evaluated NPC town had loyalty below the native threshold, so **genuine Festival substitution was not observed** and no native commit after such a substitution is claimed. This is an honest bounded target-branch null, not a reason to retune the threshold or force conditions. Runtime telemetry commit: `f0fe5b86aa71bd0df349d05858fc6ab203fcf30b`; runtime DLL SHA-256: `2909ADE31B427E122FDDE539229BD0A4A2B4FA91C84CFD3AB63D0C8EB6F729D7`. Zero save commands and telemetry errors; the protected fixture/rollback/config remained intact and the run exited without saving. Phase 6-v1 status: **implemented YES; build-proven YES; runtime-observed YES; genuine Festival substitution NO; native final commit YES for observed passthrough choices, but substituted-Festival commit NO; player-legible NOT YET PROVEN; balance-proven NO**. See `Reports/SettlementSociety/PHASE6_CIVIC_PROJECT_V1_RUNTIME_RESULT.md`.
