...
The current ClanAI source corresponds to the Phase 2B implementation candidate `v0.22A-ruler-courtship-native-v1`. It builds successfully against the local Bannerlord assemblies and is runtime-proven: Vlandia selected naturally independent Banu Ruwaid using unmodified native values, invoked only the native join barter, and the clan joined Vlandia. The last fully demo-gated integrated candidate remains `v0.21M-player-visibility-v1`. Phase 2C is now active; its native capability audit found that independent NPC clans retain finance and party rebuilding, while ordinary independent NPC clans are excluded from native military target scoring. The separate target-kingdom defection boundary remains timeboxed and unproven.
