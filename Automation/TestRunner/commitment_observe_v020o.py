...
TARGET_SAVE = SAVES / "ClanAI v020O COMMITMENT OBSERVE TEST.sav"
...
def newest_session(after_ts):
    candidates = [
        p for p in SESSIONS.glob("*_v020O_review.log")
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("StrategicCommitment_v020O_ObserveAuto_" + stamp)
    ...
    manifest = {
        ...
        "modules": modules,
        "command": cmd,
        "stop_condition": "commitmentWouldRetain>=2 with zero applied/retains, or any composer/blackboard/commitment failure, or timeout",
    ...
    if not session:
        raise RuntimeError("No new v0.20O ClanAI session log appeared")
    ...
    while time.time() - start_run < MAX_RUN_SECONDS:
        ...
        if summaries:
            ...
            blackboard_mapping = {
                "blackboardActors": "actors",
                "blackboardDecisions": "decisions",
                "blackboardStable": "stable",
                "blackboardObjectiveSwitches": "objective_switches",
                "blackboardStateSwitches": "state_switches",
                "blackboardPrevEligibleSwitches": "prev_eligible_switches",
                "blackboardPrevMissingSwitches": "prev_missing_switches",
                "blackboardNear5": "near5",
                "blackboardNear10": "near10",
                "blackboardNear20": "near20",
        ...
        if blackboard["failures"]:
            stop_reason = "BLACKBOARD_FAILURE"
            break
        ...
        if commitment["would_retain"] >= 2:
            stop_reason = "COMMITMENT_OBSERVE_GATE_REACHED"
            break
