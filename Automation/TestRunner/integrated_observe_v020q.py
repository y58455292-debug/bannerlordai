...
TARGET_SAVE = SAVES / "ClanAI v020Q INTEGRATED OBSERVE TEST.sav"
...
EXPECTED_LOCK_OWNER = "sol_integrated_commitment_v020Q"
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("IntegratedCommitment_v020Q_ObserveAuto_" + stamp)
    ...
    manifest = {
        ...
        "stop_condition": ">=100 perception captures/validations with reuse+reads, >=100 intent decisions, >=100 commitment evaluations, composer owner writes>0, Observe writes=0, zero mismatches/failures, or timeout",
    ...
    while time.time() - start_run < MAX_RUN_SECONDS:
        ...
        if commitment["applied"] or commitment["actual_retains"]:
            stop_reason = "OBSERVE_WROTE_UNEXPECTEDLY"
            break
        ...
        if (
            ...
            intent["decisions"] >= 100 and
            commitment["evaluations"] >= 100 and
            commitment["prior_state_found"] > 0 and
            composer["applied_frames"] >= 100 and
            composer["score_writes"] > 0
        ):
            stop_reason = "V020Q_INTEGRATION_GATE_REACHED"
            break
