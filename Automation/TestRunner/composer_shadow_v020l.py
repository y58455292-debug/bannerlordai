...
TARGET_SAVE = SAVES / "ClanAI v020L COMPOSER SHADOW TEST.sav"
...
def newest_session(after_ts):
    candidates = [
        p for p in SESSIONS.glob("*_v020L_review.log")
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("StrategicDecisionComposer_v020L_ShadowAuto_" + stamp)
    ...
    manifest = {
        ...
        "modules": modules,
        "command": cmd,
        "stop_condition": "composerAppliedFrames>=100 with proposal>=1, or mismatch/failure, or timeout",
    ...
    if not session:
        raise RuntimeError("No new v0.20L ClanAI session log appeared")
