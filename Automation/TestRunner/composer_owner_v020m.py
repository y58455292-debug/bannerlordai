...
TARGET_SAVE = SAVES / "ClanAI v020M COMPOSER OWNER TEST.sav"
...
def newest_session(after_ts):
    candidates = [
        p for p in SESSIONS.glob("*_v020M_review.log")
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("StrategicDecisionComposer_v020M_OwnerAuto_" + stamp)
    ...
    manifest = {
        ...
        "modules": modules,
        "command": cmd,
        "stop_condition": "composerAppliedFrames>=100 with scoreWrites>0 and proposal>=1, or mismatch/failure, or timeout",
    ...
    if not session:
        raise RuntimeError("No new v0.20M ClanAI session log appeared")
    ...
    while time.time() - start_run < MAX_RUN_SECONDS:
        ...
        if summaries:
            ...
            for raw in latest.split():
                ...
                try:
                    composer[mapping[key]] = int(value)
                except Exception:
                    pass
        ...
        if (
            ...
            composer["score_writes"] > 0 and
            composer["proposal_contributions"] >= 1
        ):
            stop_reason = "EQUIVALENCE_GATE_REACHED"
            break
