...
TARGET_SAVE = SAVES / "ClanAI v020N BLACKBOARD SHADOW TEST.sav"
...
def newest_session(after_ts):
    candidates = [
        p for p in SESSIONS.glob("*_v020N_review.log")
...
def main():
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("ActorStrategicBlackboard_v020N_ShadowAuto_" + stamp)
    ...
    manifest = {
        ...
        "modules": modules,
        "command": cmd,
        "stop_condition": "blackboardDecisions>=500 and objectiveSwitches>=30, or composer/blackboard failure, or timeout",
    ...
    if not session:
        raise RuntimeError("No new v0.20N ClanAI session log appeared")
    ...
    while time.time() - start_run < MAX_RUN_SECONDS:
        ...
        if (
            blackboard["decisions"] >= 500 and
            blackboard["objective_switches"] >= 30
        ):
            stop_reason = "BLACKBOARD_SAMPLE_REACHED"
            break
