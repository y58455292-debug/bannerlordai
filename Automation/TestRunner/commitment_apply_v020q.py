...
TARGET_SAVE = SAVES / "ClanAI v020Q COMMITMENT APPLY TEST.sav"
...
EXPECTED_CLANAI_SHA = "0C9EC39ACBE7ABCA60D97C9C43B5138D39D5E0DE9B0EA1D10F44D7DA8258EC59"
EXPECTED_LOCK_OWNER = "sol_commitment_apply_v020Q"
...
COMMITMENT_CFG = ROOT / "Data/StrategicCommitment.cfg"
MAX_RUN_SECONDS = 240
_original_focus_setting = None
_original_commitment_cfg = None
...
def set_commitment_mode(mode):
    text = COMMITMENT_CFG.read_text(encoding="utf-8-sig", errors="replace")
    lines = text.splitlines()
    changed = False
    for i, line in enumerate(lines):
    ...
    COMMITMENT_CFG.write_text("\n".join(lines) + "\n", encoding="utf-8")

def restore_commitment_config():
    global _original_commitment_cfg
    if _original_commitment_cfg is None:
        return
    original = _original_commitment_cfg
    _original_commitment_cfg = None
    try:
        COMMITMENT_CFG.write_text(original, encoding="utf-8")
    except Exception:
        pass
...
def newest_session(after_ts):
    candidates = [
        p for p in SESSIONS.glob("*_v020Q_review.log")
...
def main():
    global _original_focus_setting, _original_commitment_cfg
    ...
    _original_commitment_cfg = COMMITMENT_CFG.read_text(
        encoding="utf-8-sig",
        errors="replace",
    )
    if "Mode=Observe" not in _original_commitment_cfg:
        raise RuntimeError("Expected Observe commitment config before Apply treatment")
    set_commitment_mode("Apply")
    atexit.register(restore_commitment_config)
    ...
    validation = ROOT / "Longitudinal/LiveValidation" / ("IntegratedCommitment_v020Q_ApplyAuto_" + stamp)
    ...
    manifest = {
        ...
        "stop_condition": "first causal actual retain, then >=20 additional commitment evaluations and >=30s total runtime; zero composer/perception/intent/commitment failures; or timeout",
    ...
    if not session:
        raise RuntimeError("No new v0.20Q ClanAI session log appeared")
    ...
    intent = {
        ...
        "state_switches": 0,
        "prev_eligible_switches": 0,
        "prev_missing_switches": 0,
        "near5": 0,
        "near10": 0,
        "near20": 0,
        "failures": 0,
    ...
    commitment = {
        ...
        "prior_state_found": 0,
        "natural_changes": 0,
        "cross_state_skipped": 0,
        "age_expired": 0,
        "previous_missing": 0,
        "same_state_eligible": 0,
        "would_retain": 0,
        "applied": 0,
        "actual_retains": 0,
        "failures": 0,
    }
    first_retain_evaluation = None
    apply_lines = []
    start_hours = None
    last_hours = None
    max_auto_resumes = 0
    ...
    while time.time() - start_run < MAX_RUN_SECONDS:
        ...
        if summaries:
            ...
            intent_mapping = {
                "intentBlackboardActors": "actors",
                "intentBlackboardDecisions": "decisions",
                "intentBlackboardStable": "stable",
                "intentBlackboardObjectiveSwitches": "objective_switches",
                "intentBlackboardStateSwitches": "state_switches",
                "intentBlackboardPrevEligibleSwitches": "prev_eligible_switches",
                "intentBlackboardPrevMissingSwitches": "prev_missing_switches",
                "intentBlackboardNear5": "near5",
                "intentBlackboardNear10": "near10",
                "intentBlackboardNear20": "near20",
                "intentBlackboardFailures": "failures",
            }
            commitment_mapping = {
                "commitmentEvaluations": "evaluations",
                "commitmentPriorStateFound": "prior_state_found",
                "commitmentNaturalChanges": "natural_changes",
                "commitmentCrossStateSkipped": "cross_state_skipped",
                "commitmentAgeExpired": "age_expired",
                "commitmentPreviousMissing": "previous_missing",
                "commitmentSameStateEligible": "same_state_eligible",
                "commitmentWouldRetain": "would_retain",
                "commitmentApplied": "applied",
                "commitmentActualRetains": "actual_retains",
                "commitmentFailures": "failures",
            }
            ...
            for raw in latest.split():
                ...
                try:
                    ...
                    elif key in intent_mapping:
                        intent[intent_mapping[key]] = int(value)
                    elif key in commitment_mapping:
                        commitment[commitment_mapping[key]] = int(value)
                except Exception:
                    pass
        ...
        if blackboard["mismatches"] or blackboard["failures"]:
            stop_reason = "PERCEPTION_BLACKBOARD_MISMATCH"
            break

        if intent["failures"]:
            stop_reason = "INTENT_BLACKBOARD_FAILURE"
            break

        if commitment["failures"]:
            stop_reason = "COMMITMENT_FAILURE"
            break

        if commitment["applied"] > commitment["actual_retains"]:
            stop_reason = "APPLY_DID_NOT_RETAIN"
            break

        apply_lines = [
            line for line in text_now.splitlines()
            if "STRATEGIC_COMMITMENT_APPLY " in line
        ][-20:]

        if commitment["actual_retains"] > 0 and first_retain_evaluation is None:
            first_retain_evaluation = commitment["evaluations"]

        if (
            first_retain_evaluation is not None and
            commitment["evaluations"] >= first_retain_evaluation + 20 and
            time.time() - start_run >= 30.0
        ):
            stop_reason = "COMMITMENT_APPLY_CAUSAL_RETAIN_CONFIRMED"
            break
    ...
    log(
        ...
        " perception=" + json.dumps(blackboard, sort_keys=True) +
        " intent=" + json.dumps(intent, sort_keys=True) +
        " commitment=" + json.dumps(commitment, sort_keys=True) +
    ...
    result = {
        "reason": reason,
        "composer": composer,
        "perception_blackboard": blackboard,
        "intent_blackboard": intent,
        "commitment": commitment,
        "first_retain_evaluation": first_retain_evaluation,
    ...
    restore_commitment_config()
    result["commitment_config_restored"] = COMMITMENT_CFG.read_text(
        encoding="utf-8-sig",
        errors="replace",
    )
