...
This checkpoint proves campaign registration and active-kingdom initial reconciliation. The original bounded observation advanced 76.401 campaign hours with no native lifecycle callback (honest null). The subsequent retry did not reach a valid runtime, so it adds no natural-event observation time. The save/load acceptance item and natural-notice/event item remain open. Do not claim Phase 2D-L1 complete until those are verified. No native ruler, faction, kingdom, settlement, war, or timer state was changed by this behavior.
