...
namespace BannerlordInspector
{
    ...
    public static class AiDecisionTrace
    {
        ...
        {
            // v0.22 / Experiment 03G v2:
            // GlobalSameThinkCausalCapture scans every eligible independent lord
            // AFTER all vanilla scorers have finished. With no one-shot marker it
            // is strictly read-only. If the dedicated one-shot marker exists, the
            // first genuine natural flip opportunity is modified in THIS SAME
            // PartyThinkParams object, the marker is consumed immediately, and
            // vanilla then performs its normal selection/commit.
            try
            {
                GlobalSameThinkCausalCapture.Observe(__0, __1);
            }
            catch (Exception ex)
            {
                InspectorLog.Error("GLOBAL SAME-THINK CAUSAL v0.1 observe failed.", ex);
            }
        ...
        public static void AfterPartyThink(MobileParty __0)
        {
            try
            {
                GlobalSameThinkCausalCapture.ObservePartyThinkEnd(__0);
            }
            catch (Exception ex)
            {
                InspectorLog.Error("GLOBAL SAME-THINK CAUSAL v0.1 end-of-think capture failed.", ex);
            }
            ...
            ActionCommitState __state)
        {
            try
            {
                GlobalSameThinkCausalCapture.ObserveCommit(__args);
            }
            catch (Exception ex)
            {
                InspectorLog.Error("GLOBAL SAME-THINK CAUSAL v0.1 commit capture failed.", ex);
            }
            ...
            out TransitionState __state)
        {
            ...
            // These SetMove* APIs are used in many places. We only care about the exact
            // moment PartyHourlyAiTick commits its chosen strategic result for Kanujan.
