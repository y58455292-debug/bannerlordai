...
RELEASE = Path(r"D:\BannerlordAIResearch\Tools\FieldCommander\Releases\v2.2-regional")
MODEL = RELEASE / "Model" / "kokoro-v1.0.int8.onnx"
VOICES = RELEASE / "Model" / "voices-v1.0.bin"
TOWN_MAP_PATH = RELEASE / "settlement_to_town.json"
...
LOG = ROOT / "field_commander_v2_2.log"
READY = ROOT / "v2_2_ready.txt"
AUDIO = ROOT / "field_commander_v2_2_current.wav"
CACHE = ROOT / "cache_v2_2"

POLL_SECONDS = 0.45
AUTO_COOLDOWN = 30.0
DUPLICATE_SECONDS = 180.0
...
def load_town_map():
    global town_map
    try:
        town_map = json.loads(TOWN_MAP_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        town_map = {}
        log("TOWN_MAP_ERROR " + type(exc).__name__)

def town_anchor(target):
    target = clean(target)
    if not target:
        return None
    info = town_map.get(target)
    if not info:
        return None
    town = clean(info.get("town", ""))
    return town or None
...
def build_visual_report(line):
    ...
    town = town_anchor(target)

    if not town:
        log(f"REGIONAL_SKIP unmappedTarget={target} actor={actor} reason={reason}")
        return None
    ...
    # Spoken output is deliberately ONLY the town name.
    # Actor, target and reason remain in telemetry/logs for later analysis.
    key = f"town|{town}"
    return new_report(priority, key, town + ".", 0.92)
...
def build_report(line):
    ...
    if "WORLD_MEMORY event=RaidStarted" in line:
        ...
        if not town:
            log(f"REGIONAL_SKIP unmappedRaidTarget={target} actor={actor}")
            return None

        return new_report(3, f"town|{town}", town + ".", 0.92)
    ...
    if "MEMORY_CAUSAL_RESULT" in line and "PASS_FLIPPED" in line:
        ...
        if not town:
            log(f"REGIONAL_SKIP unmappedMemoryTarget={target} party={party}")
            return None

        return new_report(6, f"town|{town}", town + ".", 0.93)
...
def main():
    ...
    log("BOOT v2.2 regional scout neural engine loading")
    load_town_map()
    log(f"TOWN_MAP_READY entries={len(town_map)}")
    ...
    log(f"READY v2.2 voice={VOICE_NAME} mode=REGIONAL_TOWNS_ONLY telemetryOffset={position} warmupMs={warm_ms:.0f}")
